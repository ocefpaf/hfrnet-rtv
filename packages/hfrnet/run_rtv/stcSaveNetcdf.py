...
def stcSaveNetcdf(metaDataObj, configObj, fileLocObj, stcInfoObj, rtvProcessObj,
                  time, A_tot, inputs):
    '''
    % STCSAVENETCDF Saves STC data to a NetCDF file
    %
    %    stcSaveNetCDF(metaDataObj, configObj, fileLocObj, stcInfoObj, rtvProcessObj,
                       time, A_tot )
    %    saves sub-tidal current solutions and corresponding metadata in
    %    NetCDF format using the Climate and Foecast (CF) and Attribute Convention
    %    for Data Discovery (ACDD) metadata standards.
    %
    ...
    # Save
    try:
        writeNetCDF(metaDataObj, configObj, fileLocObj, stcInfoObj,
                    rtvProcessObj, time, A_tot, inputs)
...
def writeNetCDF(metaDataObj, configObj, fileLocObj, stcInfoObj, rtvProcessObj,
                t, A, inputs):
    """ Formats STC data and writes NetCDF file """
    ...
    hi_uvar_count = np.sum(np.sum(u_var > 32767))  # 16-bit signed limit
    u_var[u_var > 32767] = 32767 # hi limit range check
    if hi_uvar_count > 0:
        _log.notice(f'U variance exceeds integer limits {hi_uvar_count} times while saving NetCDF' )

    hi_vvar_count = np.sum(np.sum(v_var > 32767))  # 16-bit signed limit
    v_var[ v_var > 32767 ] = 32767 # hi limit range check
    if hi_vvar_count > 0:
        _log.notice(f'V variance exceeds integer limits {hi_vvar_count} times while saving NetCDF')
    ...
    # ID (ACDD)
    ncid.id_HFR = rtvNcid(metaDataObj, configObj, rtvProcessObj, t)
    ...
    # u_mean
    varid_u_avg = ncid.createVariable('u_mean', 'i2', reversed([dimid_lon, dimid_lat, dimid_t]),
                                      compression=compression,
                                      complevel=complevel,
                                      shuffle=shuffle,
                                      fill_value=fill)
    ...
    varid_u_avg.cell_methods = 'depth: mean time: mean (interval: 1 hour comment: 1 hour average)'
    ...
    # v_var
    varid_v_var = ncid.createVariable('v_var', 'i2', reversed([dimid_lon, dimid_lat, dimid_t]),
                                      compression=compression,
                                      complevel=complevel,
                                      shuffle=shuffle,
                                      fill_value=fill)
    ...
    # u_min
    varid_u_min = ncid.createVariable('u_min', 'i2', reversed([dimid_lon, dimid_lat, dimid_t]),
                                      compression=compression,
                                      complevel=complevel,
                                      shuffle=shuffle,
                                      fill_value=fill)
    ...
    varid_u_min.cell_methods = ('depth: mean time: minimum (interval: 1 hour '
                                'comment: 1 hour average)')
    varid_u_min.ancillary_variables = 'n_obs'

    # v_min
    varid_v_min = ncid.createVariable('v_min', 'i2',
    ...
    # u_max
    varid_u_max = ncid.createVariable('u_max', 'i2', reversed([dimid_lon, dimid_lat, dimid_t]),
                                      compression=compression,
                                      complevel=complevel,
                                      shuffle=shuffle,
                                      fill_value=fill)
    ...
    varid_u_max.cell_methods = ('depth: mean time: maximum (interval: 1 hour '
                                'comment: 1 hour average)')
    ...
    varid_v_max.cell_methods = ('depth: mean time: maximum (interval: 1 hour '
                                'comment: 1 hour average)')
    ...
    # STC Processing parameter 'variable'
    varid_pp = ncid.createVariable('processing_parameters', 'i1')

    varid_pp.long_name = 'Methods and parameters used to compute subtidal current'
    ...
    varid_pp.min_temporal_coverage = np.int16(stcInfoObj.min_temporal_coverage)
    varid_pp.min_temporal_coverage_description = \
        'Minimum number of hours required to compute a subtidal current'

    if rtvProcessObj.method == 'uwls':
        varid_pp.max_hdop = np.float32(stcInfoObj.max_error)
    ...
    varid_t[0] = t.timestamp()
    varid_time_bnds[:] = np.array([(t -  timedelta(hours=12.5)).timestamp(),
                                   (t + timedelta(hours=12.5)).timestamp()]).reshape(1, 2) # 25 hrs
